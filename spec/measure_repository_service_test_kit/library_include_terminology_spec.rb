...
RSpec.describe MeasureRepositoryServiceTestKit::LibraryIncludeTerminology do
  ...
  describe 'Server successfully returns bundle on Library $package with id in url and include-terminology=true' do
  ...
  describe 'Server successfully returns bundle on Library $package with url in body and include-terminology=true' do
    ...
    it 'passes if a 200 is returned with bundle body and url matches' do
      ...
      stub_request(
        :post,
        "#{url}/Library/$package?include-terminology=true"
      ).to_return(status: 200, body: bundle.to_json)
  ...
  describe 'Server successfully returns bundle on Library $package with identifier in
  body and include-terminology=true' do
    ...
    it 'fails if Library in returned bundle does not match identifier value' do
  ...
  describe 'Server successfully returns bundle on Library $package with url, identifier, and version in
  body and include-terminology=true' do
    ...
    it 'fails if a 200 is returned with bundle body but id does not match' do
      ...
      result = run(test, url:, library_id:, library_url:, library_identifier:,
                         library_version:)
      expect(result.result).to eq('fail')
    end
    ...
    it 'fails if a 200 is returned with bundle body but identifier system does not match' do
      library = FHIR::Library.new(id: library_id, url: library_url, identifier: { system: 'invalid_system',
    ...
    it 'fails if a 200 is returned with bundle body but identifier value does not match' do
      library = FHIR::Library.new(id: library_id, url: library_url, identifier: { system: 'identifier_system',
                                                                                  value: 'invalid_value' },
                                  version: library_version)
      bundle = FHIR::Bundle.new(total: 1, entry: [{ resource: library }])
  ...
  with include-terminology=true' do
    ...
    it 'passes if all related artifacts are present' do
      ...
      repo_create(
        ...
        url: "http://example.com/Library/#{library_id}/$package?include-terminology=true",
    ...
    it 'fails if related artifacts are missing' do
      bundle = FHIR::Bundle.new(total: 1,
                                entry: [{ resource: library }])
      repo_create(
        :request,
        name: 'library_package_include_terminology',
    ...
    it 'skips if library_package request with include-terminology=true has not been made' do
      result = run(test, url:, library_id:)
      expect(result.result).to eq('skip')
    end
  end

  describe 'Server returns 404 when no library matches id and include-terminology=true' do
    ...
    it 'passes when 404 returned with OperationOutcome' do
      stub_request(
        :post,
        "#{url}/Library/INVALID_ID/$package?include-terminology=true"
  ...
  describe 'Server returns 400 when no id, url, or identifier provided and include-terminology=true' do
