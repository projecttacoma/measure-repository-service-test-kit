...
RSpec.describe MeasureRepositoryServiceTestKit::MeasureIncludeTerminology do
  ...
  describe 'Server successfully returns bundle on Measure $package with id in url and include-terminology=true' do
  ...
  describe 'Server successfully returns bundle on Measure $package with url in body and include-terminology=true' do
    ...
    it 'passes if a 200 is returned with bundle body and url matches' do
      ...
      stub_request(
        :post,
        "#{url}/Measure/$package?include-terminology=true"
      ).to_return(status: 200, body: bundle.to_json)
  ...
  describe 'Server successfully returns bundle on Measure $package with identifier in
  ...
  describe 'Server successfully returns bundle on Measure $package with url, identifier, and version in
  body and include-terminology=true' do
  ...
  with include-terminology=true' do
    ...
    it 'passes if all related artifacts are present' do
      ...
      repo_create(
        ...
        url: "http://example.com/Measure/#{measure_id}/$package?include-terminology=true",
    ...
    it 'skips if measure_package request with include-terminology=true has not been made' do
      result = run(test, url:, measure_id:)
      expect(result.result).to eq('skip')
    end
  end

  describe 'Server returns 404 when no measure matches id and include-terminology=true' do
    ...
    it 'passes when 404 returned with OperationOutcome' do
      stub_request(
        :post,
        "#{url}/Measure/INVALID_ID/$package?include-terminology=true"
